...
@section('title', title_module('promotion','edit'))

@canany(['promotion_index', 'promotion_edit', 'promotion_destroy'])
    @section('index', route('admin.promotion.index'))
@endcanany
...
@section('content')
    <form action="{{ route('admin.promotion.update', ['promotion' => $news->uuid]) }}" method="POST" enctype="multipart/form-data" class="formAjax">
        @csrf
        @method('PUT')

        <div class="row">
            @if (auth()->user()->level == 1)
                @include('administrator.partials.button', ['exit' => route('admin.promotion.index')])                
            @endif
                            ...
                            <li class="nav-item">
                             ...
                             <li class="nav-item">
                                <a href="#card-toolbar-tab2" class="nav-link" data-toggle="tab">
                                    ...
                                    {{ behavior('action.list_product') }}
                                </a>
                            </li> 
                            <li class="nav-item">
                                <a href="#card-toolbar-tab3" class="nav-link" data-toggle="tab">
                                    <i class="icon-image2 mr-2"></i>
                                    {{ behavior('action.create_product') }}
                                </a>
                            </li> 
                        </ul>
                    </div>
                    ...
                    <div class="card-body tab-content" id="forms-target-left">
                        <div class="tab-pane fade show active" id="card-toolbar-tab1">
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-6">
                                    <x-text label="promotion.promotion_percent" type="number" name="promotion_percent" placeholder="promotion.promotion_percent">
                                        {{ old('promotion_percent', $news->promotion_percent) }}
                                    </x-text>
                                </div>
                            </div>
                            ...
                            <table class="table table-dark">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            {{number_format($item['price'] -  $item['price'] * ($news->promotion_percent/100)) }}
                                        </td>
                        ...
                        <div class="tab-pane fade" id="card-toolbar-tab3">
                            <x-product_select></x-product_select>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-4">
                ...
                <x-card title="action.info" id="forms-target-right">
                    ...
                    <x-selectbox label="promotion.role_id" name="role_id" :dataSelect="role_function()">
                        {{ old('role_id',$news->role_id) }}
                    </x-selectbox>
