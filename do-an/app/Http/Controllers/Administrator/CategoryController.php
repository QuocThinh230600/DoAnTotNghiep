...
class CategoryController extends AdminController
{
    ...
    public function index() 
    {
        ...
        if (Request::is('api*')) {
            return new Category($data['categories']);
    ...
    public function store(StoreRequest $request)
    {
        ...
        DB::transaction(function () use ($request, &$category, &$categoryTranslation) {
            $category            = $request->only('icon', 'parent_id', 'position', 'access', 'open_link','featured','link1','link2','link3','banner1','banner2','banner3');
            $category['status']  = $request->status ?? 'off';
            $category['featured'] = is_null($request->featured) ? null : $request->featured;
            ...
            $categoryTranslation                    = $request->only('name', 'description', 'image','image_mobile', 'link', 'slug', 'title_tag', 'meta_keywords', 'meta_description');
