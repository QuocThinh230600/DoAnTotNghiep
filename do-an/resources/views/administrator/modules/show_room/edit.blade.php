...
@section('title', title_module('showRoom','edit'))
...
@section('content')
    <form action="{{ route('admin.showRoom.update', ['showRoom' => $showRoom->uuid]) }}" method="POST" enctype="multipart/form-data" class="formAjax">
        @csrf
        @method('PUT')
            ...
            <div class="col-lg-4">
                <x-card title="action.info" id="forms-target-right">
                    <x-toggle label="element.status" name="status" on="element.status_enable"  off="element.status_disable" required="required">
                        {{ old('status', $showRoom->status) }}
                    </x-toggle>

                    <x-image name="image">
                        {{ old('image', $showRoom->image) }}
                    </x-image>
                </x-card>
            </div>
