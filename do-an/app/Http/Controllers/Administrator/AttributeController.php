...
class AttributeController extends AdminController
{
    ...
    {
        $data['attribute']    = $this->attribute->getAllAttributeRecursive();
    ...
    {
        $attribute = $this->attribute->query()->where('id', '<>', 1)->whereNull('attributes.deleted_at');
        ...
        return Datatables::of($attribute)
            ...
            ->editColumn('status', function ($attribute) {
                if ($attribute->status == 'on') {
                    $xhtml = '<span class="badge btn-xs badge-info">' . label('element.status_enable_filter') . '</span>';
                } else {
                    $xhtml = '<span class="badge badge-secondary">' . label('element.status_disable_filter') . '</span>';
