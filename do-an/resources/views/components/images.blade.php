        ...
        @foreach($dataImg as $item)
            <div class="row mb-2 row-upload-multi" data-id="{{ $loop->iteration }}" id="row-upload-multi-image-{{ $loop->iteration }}">
                ...
                <div class="col-md-3 my-auto">
                    <select name="multi_images[{{ $loop->iteration }}][image_position]" class="form-control">
                        <option value="1" {{$item->image_position == 1 ? 'selected' : ''}}>Ảnh phụ</option>
                        <option value="2" {{$item->image_position == 2 ? 'selected' : ''}}>Ảnh thông số kĩ thuật</option>
                        <option value="3" {{$item->image_position == 3 ? 'selected' : ''}}>Ảnh mô tả</option>
                    </select>
                </div>
