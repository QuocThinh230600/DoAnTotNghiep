    ...
    <x-card title="action.info" table="true">
        <table class="table table-hover datatable-colvis-basic" url="{{ route('admin.ajax.attributeDataTables') }}">
            ...
            <tr>
                ...
                <th type="text" data="name" name="name" orderable="false" >{{ label('attribute.name') }}</th>
                <th type="text" data="parent" name="parent" orderable="false" >{{ label('attribute.parent') }}</th>
                <th type="select" data="status" name="status">
                    <select class="form-control">
                        <option value="">{{ label('please_choose') }}</option>
                        <option value="on">{{ label('element.status_enable_filter') }}</option>
                        <option value="off">{{ label('element.status_disable_filter') }}</option>
                    </select>
                </th>
                @canany(['attribute_edit', 'attribute_destroy'])
