...
class ReplyContactController extends AdminController
{
    ...
    public function send_reply(SendReplyRequest $request, string $uuid)
    {
        ...
        ReplyContact::flushQueryCache(['reply_contacts']);
    ...
    public function destroy(string $uuid)
    {
        ...
        Contact::flushQueryCache(['contacts']);
    ...
    {
        $contact = $this->contact->getAllQuery();
        return Datatables::of($contact)
            ...
            ->editColumn('product_id', function ($contact) {
                return $contact->product->name;
            })
            ->addColumn('actions', function ($image) {
                return view('administrator.modules.contact.actions', ['uuid' => $image->uuid]);
            })
            ->rawColumns(['actions', 'status','product_id'])
            ->addIndexColumn()
            ->make(true);
    }
}
