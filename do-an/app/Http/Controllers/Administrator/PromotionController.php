...
use App\Http\Requests\Administrator\Promotion\StoreRequest;
use App\Http\Requests\Administrator\Promotion\UpdateRequest;
use App\Http\Requests\Administrator\Promotion\TranslationRequest;

class PromotionController extends AdminController
{
    private $view = 'administrator.modules.promotion.';

    private $route = 'admin.promotion.';

    private $module = 'module.promotion';
    ...
    public function __construct(LanguageRepository $language,
        ...
        ProductTranslationRepository $productTranlate,
        NewsTranslationRepository $newsTranslation) {
        parent::__construct();
        $this->middleware('permission:promotion_index', ['only' => ['show', 'index', 'dataTableIndex']]);
        $this->middleware('permission:promotion_create', ['only' => ['create', 'store', 'language', 'translation']]);
        $this->middleware('permission:promotion_edit', ['only' => ['edit', 'update']]);
        $this->middleware('permission:promotion_destroy', ['only' => ['destroy']]);

        $this->language = $language;
        $this->productTranlate = $productTranlate;
        $this->news = $news;
    ...
    public function store(StoreRequest $request)
    {
        ...
        DB::transaction(function () use ($request, &$news, &$newsTranslation) {
            $news = $request->only('date_start', 'time_start', 'position','role_id', 'open_link', 'status', 'featured', 'template', 'viewed');
            ...
            if ($request->multi_attribute) {
                foreach ($request->multi_attribute as $row) {
                    $product                   = $this->productTranlate->getByProductId($row['attribute']);
                    $data['promotion_id']      = $news->id;
                    $product->update($data);    
                }
            }


            $newsTranslation = $request->only('title', 'promotion_percent', 'author', 'copyright', 'intro', 'content', 'foot', 'image', 'youtube', 'file', 'slug', 'title_tag', 'meta_keywords', 'meta_description');
    ...
    public function edit(string $uuid)
    {
        ...
        $product = $this->productTranlate->getAllProductByPromotion($news['news']['id']);
    ...
    public function update(UpdateRequest $request, string $uuid)
    {
        ...
        if(auth()->user()->level == 1)
        {
            DB::transaction(function () use ($request, $uuid, &$news, &$newsTranslation) {
                $news = $request->only('date_start', 'time_start','role_id', 'position', 'open_link', 'status', 'featured', 'template', 'viewed');
                $news['access'] = is_null($request->access) ? null : implode(",", $request->access);
                $news['featured'] = is_null($request->featured) ? null : implode(",", $request->featured);
                $news['user_id'] = auth()->user()->id;
        ...
        return response()->json(
            [
                ...
                'redirect' => route($this->route . 'edit', ['promotion' => $uuid]),
                'result' => array('news' => $news, 'newsTranslation' => $newsTranslation),
            ], 200);
    }
    ...
    {
        $news = $this->news->getAllNewsByCateId(1);
        if(auth()->user()->level != 1){
            $news->where('role_id',auth()->user()->level);
        }
        return Datatables::of($news)
            ...
            ->editColumn('status', function ($news) {
                if($news->date_end){
                        ...
                        $date = DateTime::createFromFormat('d/m/Y', $news->date_end);
                        ...
                        {
                            if ($datediff > $now) {
                                $xhtml = '<span class="badge btn-xs badge-info">' . label('promotion.enble_status') . '</span>';
                            } else{
                                $xhtml = '<span class="badge badge-warning">' . label('promotion.disable_time') . '</span>';
                            }
                        }else{
                            $xhtml = '<span class="badge badge-secondary">' . label('promotion.disable_status') . '</span>';
                        }
                }else{
                        $xhtml = '<span class="badge btn-xs badge-info">' . label('promotion.enble_status') . '</span>';
                }
            ...
            ->editColumn('date_',function($news){
                if($news->role_id == 2){
                    return label('user.staff');
                }elseif($news->role_id == 3){
                    return label('user.collaborators');
                }else{
            ...
            ->addColumn('actions', function ($news) {
                return view('administrator.modules.promotion.actions', ['uuid' => $news->uuid, 'news' => $this]);
            })
